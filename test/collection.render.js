...
describe('collection.render', function() {
  describe('rendering', function() {
    ...
    it('should throw an error when an engine is not defined', function(cb) {
      ...
      views.render(page, function(err) {
    ...
    it('should use helpers to render a view', function(cb) {
      ...
      views.helper('upper', function(str) {
        return str.toUpperCase(str);
      });

      views.addView('a.tmpl', {content: 'a <%= upper(name) %> b', locals: locals});
      var page = views.getView('a.tmpl');

      views.render(page, function(err, res) {
    ...
    it('should use globally defined data to render a view', function(cb) {
      views.data({name: 'Halle'});

      views.helper('upper', function(str) {
        return str.toUpperCase(str);
      });

      views.addView('a.tmpl', {content: 'a <%= upper(name) %> b'});
      var page = views.getView('a.tmpl');
    ...
    it('should render a template when contents is a buffer', function(cb) {
      views.addView('a.tmpl', {content: '<%= a %>', locals: {a: 'b'}});
      var view = views.getView('a.tmpl');

      views.render(view, function(err, view) {
        if (err) return cb(err);
    ...
    it('should render a view from its path', function(cb) {
      ...
      views.render('a.tmpl', function(err, view) {
        if (err) return cb(err);
    ...
    it('should use a plugin for rendering', function(cb) {
      ...
      views.renderEach(function(err, items) {
        if (err) return cb(err);
