...
function parseFile (filepath, obj, options) {
  ...
  file = parseMatter(file, 'utf8', options);
  file = parsePath(file, 'utf8', options);
  file = extendFile(file, 'utf8', options);
  return file;
}

function parseString (obj, options) {
  debug('parseString', arguments);
  var contents = obj.contents ? obj.contents : new Buffer(obj.content);
  var file = new File({
    path: options.filepath || obj.path || obj.filepath,
    contents: contents
  });
  for (var key in obj) {
    if (obj.hasOwnProperty(key) && !file.hasOwnProperty(key)) {
