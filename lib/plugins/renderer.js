...
module.exports = function rendererPlugin(options, locals) {
  ...
  if (taskName) {
    ...
    buildKey = function (file) {
      return path.basename(file.path, path.extname(file.path));
    };
  }
  var plural = assemble.subtype[fileType];
  ...
  return through.obj(function(record, encoding, cb) {
    ...
    try {
      ...
      var file = assemble.cache[plural][key];
      if (!file) {
        stream.push(record);
        return cb();
      }
      ...
      file.render(locals, function(err, content) {
        ...
        record.path = gutil.replaceExtension(record.path, file.options.destExt || file.ext);
        stream.push(record);
        cb();
      });
