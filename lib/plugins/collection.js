...
var makeRelatedPages = require('./collections/related-pages');
var makeIndexPages = require('./collections/index-pages');

module.exports = function (assemble) {
  function makeCollections (cols, options) {
    if (!cols || !cols.length) {
      return through.obj();
    }
    ...
    return through.obj(function (file, enc, callback) {
      ...
      collections.forEach(function (collection) {
        ...
        makeIndexPages(assemble)(collection, indexOpts, function (err, indexFiles) {
          if (err) {
          ...
          makeRelatedPages(assemble)(collection, relatedOpts, function (err, files) {
            if (err) {
              stream.emit('error', new gutil.PluginError('assemble-collections', err));
              return callback();
            }
