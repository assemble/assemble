...
module.exports = function (assemble, params) {
  ...
  assemble.context = function () {
    ...
    // merge the params.context and page.metadata over assemble.options
    params.context = _.merge({}, assemble.options, params.context, (params.page ? params.page.metadata : {}));

    // make sure the params.context contains the metadata for the current page
    // add more aliases here
    if (params.page) {
      params.context.page = params.page.metadata;
    }

    var omitted = _.omit(params.context, ['grunt', 'orig', 'pages']);
    var processed = data.process(omitted, params.context, { imports: { grunt: assemble.options.grunt } });
