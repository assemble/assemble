...
Config.prototype.visit = function visit(fn) {
  ...
  for (var key in this.cache) {
    if (this.hasOwn(key)) {
      ...
      if (child != null
          && typeof utils.typeOf(child) === 'object'
          && !Array.isArray(child)) {
...
Config.prototype.set = function set(key, value, resolve) {
  if (arguments.length === 1 && utils.typeOf(key) === 'object') {
    key = expand(this.cache, key);
    this.extend(key);
    return this;
  }

  if (utils.typeOf(value) === 'object') {
    _.extend(this.cache, value);
  }
...
Config.prototype.process = function process(lookup, opts) {
  ...
  this.extend(expand(this.cache, lookup, opts));
  return this;
};
