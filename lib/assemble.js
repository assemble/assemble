...
Assemble.prototype.transformer = function(ext, options) {
  ...
  if (options.render && typeof options.render === 'function'
      || options.renderFile && typeof options.renderFile === 'function') {

    _.methods(options).forEach(function(key) {
      console.log(key);
      engine[ext][key] = options[key];
    });
    options.loadModule(options.name);
     //  engine[ext].render = options.render;
     //  engine[ext].renderFile = options.renderFile || fn.__express;
      // engine[ext].options = _.extend({}, options.options);
