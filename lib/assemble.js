...
Assemble.prototype.defaultTemplates = function() {
  this.template('page', {plural: 'pages'});
  this.template('partial', {plural: 'partials'});
  this.template('layout', {plural: 'layouts', isLayout: true});
};
...
Assemble.prototype.defaultEngines = function() {
  ...
  this.engine('hbs', require('./engine/handlebars'), { destExt: '.html', layoutDelims: ['{%', '%}'] });
  this.engine('md', require('./engine/handlebars'), { destExt: '.html', layoutDelims: ['{%', '%}'] });
};
...
/**
 * Add a new template type to Assemble.
 * Creates methods for adding templates to a cache based on the type name.
 *
 * **Example:**
 *
 * ```js
...
Assemble.prototype.template = function(type, options) {
  var opts = _.extend({}, options);
  if (!opts.plural) {
    throw new Error('Plural must be supplied on the options.');
  }
  ...
  Assemble.prototype[plural] = function (patterns, options) {
    ...
    _.forIn(o, function(file, key) {
      ...
      if (isLayout) {
        ...
        this._addLayout(file.ext || file.src.ext || file.data.ext, key, file, opts);
      }
    }.bind(this));
...
Assemble.prototype.render = function (file, options, callback) {
  ...
  var data = _.extend({}, this.cache.locals, this.cache.data, this.context.get('partials'));
