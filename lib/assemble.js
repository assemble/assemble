...
Assemble.prototype.hasOwn = function (key) {
  return {}.hasOwnProperty.call(this.cache, key);
};



Assemble.prototype.each = function (fn) {
...
Assemble.prototype.visit = function (fn) {
  var cloned = {};
  for (var key in this.cache) {
    if (this.hasOwn(key)) {
...
Assemble.prototype.src = function (glob, options, data) {
  ...
  return utils.src.call(this, glob, options, data);
};
...
Assemble.prototype.dest = function (dest, options) {
  ...
  return utils.dest.call(this, dest, options);
};
...
Assemble.prototype.data = function(data, options, process) {
  if (!arguments.length) {
    return this.get('data');
  }

  // TODO: if `true` is passed as a second or third param,
  // process the config data with expander.
  //
  // if (!arguments.length === 2 && typeof options === "boolean") {
  //   return this.get('data');
  // }

  var obj = this.plasma(data, options);
  return _.extend({}, this.mergeRoot(obj), obj);
};
...
Assemble.prototype.mergeRoot = function(data) {
  // var data = this.set('data', data || {});
  data = data || {};
...
Assemble.prototype.extendData = function(config, options) {
  Assemble.__super__.extend.call(this, this.data(config, options));
};
...
Assemble.prototype.plasma = function(data, options) {
  this.extend({}, plasma(data, options));
  return this;
};
...
Assemble.prototype.namespace = function(data, options) {
  data = plasma({namespace: ':basename', patterns: data}, options);
  Assemble.__super__.extend.call(this, data);
};
