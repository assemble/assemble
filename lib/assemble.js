...
Assemble.prototype.init = function(options) {
  ...
  this.set('partials', new Views(options));
  this.set('layouts', new Views(options));
...
//   enumerable: true,
//   get: function () {
//     console.log(this.options.partials)
...
//     var patterns = arrayify(options.layouts || []);
//     var ext = options.layoutext || this.get('view engine');

//     if (options.layoutdir) {
//       var layoutpath = path.join(options.layoutdir, '*') + (ext ? ext : '.*');
//       patterns.push(layoutpath);
//     }
//     layouts.cache(options.layouts || [], options);
//     return layouts.get();
//   },
//   set: function (value) {
//     layouts = new Views(value, this.options);
//   }
// });
...
Assemble.prototype.route = function (srcOptions) {
  var opts = _.extend({}, this.options, srcOptions);
