...
assemble.defaults = _.extend({
  plugins: [
    path.join(assemble.paths.plugins, 'paths.js'),
    path.join(assemble.paths.plugins, 'pagination.js')
  ],
  middleware: [
    middleware.config,
    middleware.data,
...
App.prototype.registerHelpers = function(pattern, done) {
  if (!this.engine) {
    return done(new Error('The rendering engine has not be configured yet. Register helpers after configuration is finished.'));
  }
