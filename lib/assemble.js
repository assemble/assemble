...
var LayoutEngine = require('./view/layout-engine');
...
var page = require('./plugins/pages');
...
function Assemble(options) {
  ...
  // make sure a new instance of LayoutEngine is created
...
Assemble.prototype.pages = function(options) {
  var pages = this.options.pages || [];
  _.extend(options, {pages: pages});
...
Assemble.prototype.layouts = function (patterns, options) {
  patterns = this._normalizeLayouts(patterns, options);
 ...
 * ## ._normalizeLayouts
 *
 * Use layout options to combine the patterns to make glob patterns
 * for looking up layouts.
 *
 * 
...
Assemble.prototype._normalizeLayouts = function (patterns, options) {
  options = _.extend({}, this.options, options);
  patterns = _.union([], patterns, options.layouts);
  if (options.layoutdir) {
    var layoutpath = path.join(options.layoutdir, '*') + (options.layoutext ? options.layoutext : '.*');
...
Assemble.prototype.engine = function(ext, options, fn) {
  ...
  // Create a `LayoutEngine` instance for this engine (`ext`)
  var layoutEngine = new LayoutEngine(this);
  options = _.extend({delims: delimsMap(utils.trim(ext, '.'))}, options);

  // Pass options to the current engine.
  layoutEngine.set(ext, options);
  return this;
};
