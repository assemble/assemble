...
// Local modules.
var defaultPlugins = require('./defaults/plugins');
...
Assemble.prototype.src = function (glob, options) {
  ...
  return es.pipe.apply(es, utils.arrayify([
    fs.src(glob, options),
    // defaultPlugins.src(this)(glob, options)
...
Assemble.prototype.dest = function (dest, options) {
  ...
  return es.pipe.apply(es, utils.arrayify([
    // defaultPlugins.dest(this)(dest, options),
...
//   this.partialsCache.add(this.options.partials);
...
defineGetter('layouts', function () {
  ...
  var layouts = this.layoutsCache;
  layouts.add(patterns, this.options);
  return layouts.get();
});
 ...
 * ## .route
 *
 * Traverse the middleware stack, running the `file` object through
 * each middleware function and returning the accumlated result.
 *
 * @param  {Object} `options`
 * @api private
 */
