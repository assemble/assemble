...
// add assemble plugin stages to Pluggable
Pluggable.plugins.addStages(config.plugins.stages);
...
var Line = assemble.Line = function (name, options) {
  ...
  this.engine = render.engine.get(this, this.options.engine || assemble.defaults.engine, this.options);
  this.components = [];

  this.options.plugins = _.union(this.options.plugins, assemble.defaults.plugins);
  this.pluggable = new Pluggable(this);
  this.plugins = this.pluggable.resolve(this.options.plugins, this.options);
...
Line.prototype.build = function (callback) {
  ...
  this.pluggable.run(params, callback);
};

Line.prototype.registerPlugin = function (name, description, options, fn) {
  this.pluggable.createPlugin(name, description, options, fn);
};