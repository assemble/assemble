...
Assemble.prototype.src = function (glob, options, data) {
  ...
  if(this.enabled('minimal')) {
    return fs.src(glob, this._options, data);
  }
  ...
  return lazypipe()
    .pipe(fs.src, glob, this._options, data)
    .pipe(this.parse.bind(this), this._options)
    // .pipe(this.route, dest, this._options))
    .pipe(function () {
      return peek;
    })();
};
...
Assemble.prototype.dest = function (dest, data, options) {
  var opts = _.extend({}, this._options, options);
  var self = this;

  data = data || {};

  if (this.enabled('minimal')) {
    return fs.dest(dest, this._options);
  }
  ...
  // peek = peek.pipe(fs.dest(dest, this._options));
  // peek.resume();
  // return peek;

  return lazypipe()
    .pipe(function () {return peek;})
    .pipe(this._render.bind(this), data, opts)
    .pipe(fs.dest, dest, opts)()
};
...
Assemble.prototype._render = function (data, options) {
  options = _.extend({}, this._options, options);
  data = _.extend({}, data);
  return renderer(this)(data, options);
};
    ...
    try {
      ...
      for (var i in parsers) {
        if (parsers.hasOwnProperty(i)) {
          ...
          try {
            file = parser(file, encoding, options);
          } catch (err) {
      ...
      this.emit('error', new gutil.PluginError('assemble.parse() file.contents error', err));
    }
