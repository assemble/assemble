...
defaults(Assemble.prototype, Cache.prototype);
defaults(Assemble.prototype, Data.prototype);
...
Assemble.prototype.init = function(options) {
  ...
  this.set('partials', new Template({}, options));
  this.set('layouts', new Template({}, options));
...
Assemble.prototype.defaultConfig = function (options) {
  ...
  this.enable('routes plugin');
  this.enable('buffer plugin');
  this.enable('extend plugin');
  ...
  var parserOpts = extend({autodetect: true}, self.get('front matter'));
  ...
  this.parser('md', defaultParser(parserOpts));

  // Parse front-matter from handlebars templates
  this.parser('hbs', defaultParser(parserOpts));
...
Assemble.prototype.collection = function (options) {
  ...
  return collection(this)(collections, options);
};
...
defineGetter('partials', function get() {
...
Assemble.prototype.engine = function (ext, options, fn) {
  ...
  if (typeof options === 'object') {
    if (options.render && options.renderFile) {
      ...
      engine[ext].options = extend({}, options);
    }
  }
...
Assemble.prototype.parse = function (options) {
  return parser.call(this, options);
};
