...
var registerHelpers = require('./engine/helpers');
 ...
 * ## .render
 *
 * This is Assemble's internal render method, but it's exposed as a
 * public method so it can be replaced with a custom `render` method.
 *
...
Assemble.prototype.helpers = function (patterns) {
  ...
  var helpers = registerHelpers({helpers: patterns});
  return helpers;
};
 ...
 * ## .parser
 *
 * Register a parser `fn` to be used on each `.src` file. This is
 * used to parse front matter, but can be used for any kind of parsing.
 *
 * By default, Assemble will parse front matter using [gray-matter]. For
 * front-matter in particular it is probably not necessary to register
 * additional parsing functions, since gray-matter can support almost any
 * format, but this is cusomizable if necessary or if a non-supported
 * format is required.
 *
 * **Example:**
 *
