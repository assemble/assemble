  ...
  return through.obj(function (file, encoding, callback) {
    ...
    try {
      ...
      for (var i in parsers) {
        ...
        try {
          var page = parse(str, options);
          if (page && typeof page === 'object') {
            file.original = page.original;
            if (!self.enabled('whitespace')) {
              page.content = page.content.replace(/^\s*|\s*$/gm, '');
            }
          ...
          this.emit('error', new gutil.PluginError('assemble.parse()', err));
        }
      }
