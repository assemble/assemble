...
function Assemble(context, options) {
  ...
  this.context = plasma(context || {}, this.options);
  ...
  this.disabled('minimal');
  this._engine = this._engine || this.options.engine || defaultEngine;
  this._parser = this._parser || this.options.parser || defaultParser;
...
Assemble.prototype.src = function src (glob, options, data) {
  options = options || {};
...
Assemble.prototype.dest = function dest (dest, options) {
  if(this.enabled('minimal')) {
    console.log('MINIMALLLL!')
    return fs.dest(dest, options);
  }

//  return this.use(options.engine, this.engine)(options)
//    .pipe(this.use(options.route, this.route)(dest, options))
...
Assemble.prototype.use = function use (option, middleware) {
...
Assemble.prototype.extend =
Assemble.prototype.config = function extend() {
  var args = [].slice.call(arguments);
