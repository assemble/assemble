...
assemble.init = function (source, options) {
  ...
  var results = null;
  // if requesting a new instance
  if (options.new === true) {
    // create a new one and add it to the cache
    assemble.instanceCache.put(name, new Line(options));
  } else {
    // otherwise try to get an existing instance
    results = assemble.instanceCache.get(name);
  }
