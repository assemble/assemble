...
function Assemble(options) {
  ...
  this.partial = new ViewCache(this.cache.partials, options);
  this.layout = new ViewCache(this.cache.layouts, options);
}
...
Assemble.prototype.use = function (stage, plugins) {
  if (arguments.length === 1) {
  	var current = this.plugins[stage] || [utils.passthrough];
 ...
 * ## .partial
 *
 * Get or set a partial on the cache.
 *
 *
 *
 ...
 * ```js
 * // Cache a partial
 * assemble.partial('footer', 'this is a footer.');
 * ```
 * Pass only the `key` to get a cached partial.
 *
 * ```js
 ...
 * @return {Object} Parsed `partial` object, with the following properties:
 *     @property {Object} `locals` Data from front matter
 *     @property {String} `content` The content of the file, excluding front matter.
 *     @property {String} `original` The original, unparsed string.
 * @api public
 */

//Assemble.prototype.partial = function partial(key, value) {
//	// if object
//	// if file path
//	// if string
//
...
//  }
//  return this.set('partials.' + key, value);
//};
...
Assemble.prototype.partials = function (patterns, options) {
  return this.partial.cache(patterns, options);
//  var opts = _.extend({}, this.options, options);
//  patterns = [patterns].concat(opts.partials || []);
...
Assemble.prototype.layouts = function (patterns, options) {
  return this.layout.cache(patterns, options);
