...
Assemble.prototype.defaultConfig = function () {
  ...
  // parse front matter
  this.parser('gray-matter', function (str) {
...
//  return peek;
  ...
  // var doParsing = lazypipe()
    // .pipe(renderer(self), self._engines[self.ext], data, options)

  return lazypipe()
    // .pipe(doParsing)
    .pipe(function () { return peek; })
    // .pipe(self.render, self.ext, data || {}, options)
    // .pipe(self.render, self._engines[self.ext], data || {}, options)
    .pipe(self._render, self.ext, data || {}, options)
...
// Assemble.prototype.render = function (data, options) {
//   return renderer(this._engines[this.ext], data, options);
// };

Assemble.prototype._render = function (name, data, options) {
  // data = this.extend(data);
  return this.renderer(name, data, options);
  // if (file) {
  //   var opts = {},
  //     cache = this._cache,
  ...
  //   // merge app.locals
  //   this.extend(opts, this.locals);

  //   // merge options._locals
  //   if (options._locals) {
  //     this.extend(opts, options._locals);
  //   }
  ...
  // } else {
    ...
    // TODO: Respect default view engine setting.
    // var ext = this.ext || 'hbs';
    // if ('.' !== ext[0]) {
    //   ext = '.' + ext;
    // }
    ...
    // if (engine.options) {
    //   this.extend(this._options, engine.options);
    // }
    ...
    // engine.render(name, this._options, fn);
  // }
  // return this;
};
...
Assemble.prototype.parse = function (str, options) {
  var parsers = this._parsers;

  for (var i = 0, len = parsers.length; i < len; i++) {
    var parse = parsers[i];
    try {
      var obj = parse(str, options);
