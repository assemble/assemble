...
// _.extend(Assemble.prototype, Config.prototype);
...
Assemble.prototype.stage = function (stage) {
  var stream = lazypipe();
  (this._plugins[stage] || []).forEach(function (plugin) {
    stream = stream.pipe(function () { return plugin; });
  });
  return stream;
};
...
Assemble.prototype.src = function (glob, options, data) {
  ...
  // console.log(util.inspect(this, null, 10))

  return lazypipe()
    .pipe(fs.src, glob, options)
    .pipe(this.stage('parse:before'))
    .pipe(this._parse.bind(this), options)
    .pipe(this.stage('parse:after'))
    .pipe(function () {
      return peek;
    })();
...
Assemble.prototype.dest = function (dest, options, data) {
  ...
  options = _.extend({}, {assets: this.get('assets')}, options);
  ...
  var peek = through.obj(function (file, enc, callback) {
    // console.log('dest peek', file.path);
    this.push(file);
    callback();
  });
  ...
  // peek.resume();
  // return peek;
  var ctx = _.extend({}, this.context, options);

  return lazypipe()
    .pipe(function () {return peek;})
    .pipe(this.stage('render:before'))
    .pipe(this._render.bind(this), options, data)
    .pipe(this.stage('render:after'))
    .pipe(fs.dest, route(dest, ctx), options)()
};
...
Assemble.prototype.config = function config() {
  var args = [].slice.call(arguments);
  _.extend.apply(_, [this.context].concat(args));
  return this;
};
...
Assemble.prototype._render = function (options) {
...
Assemble.prototype._parse = function (srcOptions) {
  ...
  return through.obj(function (file, encoding, callback) {
    ...
    try {
      ...
      this.emit('error', new gutil.PluginError('assemble._parse() file.contents error', err));
    }
