...
Assemble.prototype.lazyLayouts = function(ext, options) {
  if (ext && !this.layoutSettings[ext]) {
...
Assemble.prototype.layoutEngine = function(ext, engine, options) {
  if (arguments.length === 2) {
    if (typeof engine.inject !== 'function' && typeof engine.render !== 'function') {
      options = engine;
      engine = null;
    }
  }
  ...
  if (typeof engine.render !== 'function' && typeof engine.inject !== 'function') {
    throw new gutil.PluginError('Assemble', 'Layout Engines are expected to have a `render` or an `inject` method.');
  }
  ...
  this.layoutSettings[ext] = engine;
  return this;
};
 ...
 * ```
 *
 * [assemble-collections]: https://github.com/assemble/assemble-collections
 *
