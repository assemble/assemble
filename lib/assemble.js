...
var streams = require('./utils/stream');
...
Assemble.prototype.use = function (stage, plugins) {
  if (arguments.length === 1) {
    return (this._plugins[stage] || [passthrough()]);
  }
...
Assemble.prototype.src = function (glob, options, data) {
  ...
  return lazypipe()
    ...
    .pipe(drafts(this).bind(this), options)
...
Assemble.prototype.dest = function (dest, options, data) {
  ...
  return streams.join(
    ...
    this._render(options, data),
    this.use('render:after'),
    fs.dest(route(dest, ctx), options)
  );
