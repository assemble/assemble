...
assemble.paths = {
  defaults: path.join(__dirname, 'defaults.yml'),
...
assemble.defaults = _.merge({
  plugins: [
    ...
    path.join(assemble.paths.plugins, 'core-components.js'),
...
var App = assemble.App = function (options) {
  ...
  var runtimeConfig = {};
  if (options.metadata.assemblerc) {
    runtimeConfig = file.readDataSync(options.metadata.assemblerc);
  }

  this.source = options.source;

  var omissions = _.omit(this.defaults, ['plugins', 'middleware']);
  this.options = _.merge({}, omissions, options.metadata || {}, runtimeConfig || {});
 ...
 *
 * @param  {String} source  template to be compiled
