...
PluginManager.prototype.setStageCache = function (stage, plugin) {
  (this.stageCache[stage] = this.stageCache[stage] || []).push(plugin);
};

PluginManager.prototype.getStageCache = function (stage) {
  return (this.stageCache[stage] || []);
};

PluginManager.prototype.setCache = function (name, plugin) {
  this.cache[name] = plugin;
};

PluginManager.prototype.getCache = function (name) {
  return this.cache[name];
};

PluginManager.prototype.expandStages = function (pluginStages) {
  pluginStages = utils.arrayify(pluginStages);
...
PluginManager.prototype.addListeners = function (stages, plugin) {
  var self = this;
...
PluginManager.prototype.createPlugin = function (name, description, options, fn, line) {
 ...
 */
PluginManager.prototype.resolve = function (line, _plugins, options) {
  var self = this;
  options = options || {};
  _plugins = _plugins || [];

  _plugins = utils.arrayify(_plugins);
  var actualPlugins = [];
    ...
    if (_.isString(plugin)) {
      ...
      var resolvedPlugins = relPaths.map(function (relPath) {
        ...
        try {
          ...
          console.error('Error requiring plugin "' + plugin + '"');
          console.error(ex);
        }
      });
...
PluginManager.prototype.run = function (line, params, done) {
  var self = this;
...
PluginManager.prototype.runStage = function (stage, line, params, done) {
  var self = this;
