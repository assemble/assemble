...
Views.prototype.cache = function (patterns, options) {
  ...
  if (typeof patterns === 'string' && isAbsolute(patterns)) {
    if (this.viewCache.hasOwnProperty(patterns)) {
      return this.viewCache[patterns];
    }
  ...
  glob.sync(patterns, this.options).forEach(function (filepath) {
    if (this.viewCache.hasOwnProperty(filepath)) {
      return this.viewCache[filepath];
    }
...
Views.prototype.set = function (key, str, options) {
  this.viewCache[key] = this.parse(key, str, options);
  return this;
};
 ...
 *
 * ```js
 * views.setFile('templates/index.tmpl');
 * ```
 *
 * @chainable
 * @param  {String} `filepath`
 ...
 *
 * ```js
 * var foo = views.get('path/to/foo.hbs');
 * ```
 *
 * @chainable
 * @param  {String} `key` The name of the template. Typically a full file path, but can also be a name.
