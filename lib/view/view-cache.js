...
ViewCache.prototype.lookup = function (filepath) {
  if (typeof filepath !== 'string') {
    throw new Error('ViewCache#lookup() expected a string but got:', filepath);
  }
  ...
  if (isAbsolute(filepath)) {
  ...
  } else if (file.exists(path.join(cwd, filepath))) {
    ...
    throw new Error('ViewCache#lookup() cannot find:', filepath);
  }
};
 ...
 *
 * ```js
 * var viewCache = new ViewCache(options);
...
ViewCache.prototype.parse = function (name, str, options) {
  return parseFile(name, str, options);
};
