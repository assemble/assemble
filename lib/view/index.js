...
function View(file, assemble, options) {
  ...
  if (!ext) {
    file.path += (ext = this.ext = utils.ensureExt(this.defaultEngine));
  }

  this.engine = (engines[ext] || this.defaultEngine);
  ...
  // Get layouts based on the current engine.
  this.layouts = layouts.get(ext);
...
View.prototype.render = function (options, cb) {
  ...
  var context = _.extend({}, settings, this.file.locals);
