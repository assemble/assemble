  ...
  async.series([
    ...
    function (next) {
      ...
      if (_.isString(assemble.source)) {
        assemble.options.pages = assemble.options.pages || [];
        var page = null;
        if (file.exists(assemble.source)) {
          page = Component.readFile(assemble.source, 'component');
        } else {
          page = new Component({
            raw: assemble.source || ''
          });
        }
        assemble.options.pages.push(page);
      }
