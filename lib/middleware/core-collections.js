        ...
        async.eachSeries(assemble.config.collections, function (collectionOpts, next) {
          ...
          async.eachSeries(pageIndexes, function (idx, nextPage) {
            ...
            if (_.isEmpty(indexPermalinks)) {
              indexPermalinks.structure = path.join((opts.dest || '.'), collectionOpts.plural, ':num', 'index.html');
              var replacement = {
                pattern: ':num',
              ...
              indexPermalinks.replacements = [replacement];
              indexTemplate.data.dest = indexTemplate.dest = indexPermalinks.structure.replace(replacement.pattern, replacement.replacement);
            }
          ...
          async.eachSeries(collection.collectionItems.toArray(), function (collectionItem, nextCollectionItem) {
            ...
            async.eachSeries(pageIndexes, function (idx, nextPage) {
              ...
              if (_.isEmpty(relatedPermalinks)) {
                relatedPermalinks.structure = path.join((opts.dest || '.'), collectionOpts.plural, key, ':num', 'index.html');
                var replacement = {
                  pattern: ':num',
                  replacement: function () {
                    if (idx===0) {
                ...
                relatedPermalinks.replacements = [replacement];
                relatedTemplate.data.dest = relatedTemplate.dest = relatedPermalinks.structure.replace(replacement.pattern, replacement.replacement);
              }
              ...
              assemble.pages[_id] = relatedTemplate;
              nextPage();
            },
            nextCollectionItem);
          },
