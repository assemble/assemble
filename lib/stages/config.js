...
module.exports = function(assemble, done) {
  ...
  async.series([
    ...
    function (next) {
      ...
      if (assemble.helpers && assemble.helpers.length > 0) {
        var helpers = {};
        _.map(assemble.helpers, function (helperPath) {
          helpers[helperPath] = {
            name: file.basename(helperPath),
            module: require(path.normalize(path.join(process.cwd() || '', helperPath)))
          };
        });
