/*!
 * assemble <https://github.com/assemble/assemble>
 *
 * Copyright (c) 2014 Jon Schlinkert, Brian Woodward, contributors.
 * Licensed under the MIT license.
 */

'use strict';

/**
 * Module dependencies.
 */

var _ = require('lodash');
var getobject = require('getobject');


/**
 * # ConfigMonitor
 *
 * Initialize a new `ConfigMonitor` with the given `obj`.
 *
 * **Example:**
 *
 * ```js
 * var config = new ConfigMonitor();
 * ```
 *
 * @class ConfigMonitor
 * @param {Object} `config` config instance to monitor
 * @constructor
 * @api public
 */

function ConfigMonitor(config) {
  this.config = config;
  this.config.on('set', function (key, value) {
    if (typeof(value) === 'undefined') {
      this.config.staging = this.config.orig;
      this.config.cache = this.config.orig;
    } else {
      getobject.set(this.config.staging, key, value);
      getobject.set(this.config.cache, key, value);
    }
  });
}


/**
 * ## .hasOwn
 *
 * Does `this.orig` haveOwnProperty `key`?
 *
 * @method `hasOwn`
 * @param  {String} `key`
 * @return {Boolean}
 * @api private
 */

Config.prototype.hasOwn = function hasOwn(key) {
  return {}.hasOwnProperty.call(this.orig, key);
};


/**
 * ## .each
 *
 * Call `fn` on each property in `this.orig`.
 *
 * @param  {Function} `fn`
 * @api private
 */

Config.prototype.each = function each(fn) {
  for (var key in this.orig) {
    if (this.hasOwn(key)) {
      fn(key, this.orig[key]);
    }
  }
};


/**
 * ## .visit
 *
 * Traverse each _own property_ of `this.orig` and recursively
 * call `fn` on child objects.
 *
 * @param  {Function} `fn`
 * @return {Object} Return the resulting object.
 * @api private
 */

Config.prototype.visit = function visit(fn) {
  var cloned = {};
  for (var key in this.orig) {
    if (this.hasOwn(key)) {
      var child = this.orig[key];
      fn.apply(this, [key, child]);

      if (child != null
          && typeOf(child) === 'object'
          && !Array.isArray(child)) {
        child = this.visit(child, fn);
      }
      cloned[key] = child;
    }
  }
  return cloned;
};


/**
 * ## .set
 *
 * Assign `value` to `key` or return the value of `key`.
 *
 * ```js
 * assemble.set(key, value);
 * ```
 *
 * {%= docs("set") %}
 *
 * @method `set`
 * @param {String} `key`
 * @param {*} `value`
 * @param {Boolean} `resolve` Resolve template strings with [expander]
 * @return {Config} for chaining
 * @api public
 */

Config.prototype.set = function set(key, value, resolve) {
  if (arguments.length === 1 && typeOf(key) === 'object') {
    this.process(key);
    this.emit('set', key, value);
    return this;
  }

  if (typeOf(value) === 'object') {
    this.process(value);
  }

  value = expand(this.orig, value);
  if (resolve) {
    expander.set(this.orig, key, value);
  } else {
    getobject.set(this.orig, key, value);
  }

  this.emit('set', key, value);
  return this;
};


/**
 * ## .get
 *
 * Return the stored value of `key`.
 *
 * ```js
 * assemble.set('foo', 'bar');
 * assemble.get('foo');
 * // => "bar"
 * ```
 *
 * @method get
 * @param {*} `key`
 * @param {Boolean} `create` If the value doesn't exist on the orig, pass `true` to
 *                           [getobject] to initialize the value as an empty object.
 * @return {*}
 * @api public
 */

Config.prototype.get = function get(key, create) {
  if (!key) {
    return this.cache;
  }
  return getobject.get(this.cache, key, create);
};


/**
 * ## .constant
 *
 * Set a constant on the config.
 *
 * **Example**
 *
 * ```js
 * assemble.constant('site.title', 'Foo');
 * ```
 *
 * @method `constant`
 * @param {String} `key`
 * @param {*} `value`
 * @chainable
 * @api public
 */

Config.prototype.constant = function constant(key, value){
  var getter;
  if (typeof value !== 'function'){
    getter = function() {
      return value;
    };
  } else {
    getter = value;
  }
  this.cache.__defineGetter__(key, getter);
  return this;
};


/**
 * ## .enabled (key)
 *
 * Check if `key` is enabled (truthy).
 *
 * ```js
 * assemble.enabled('foo')
 * // => false
 *
 * assemble.enable('foo')
 * assemble.enabled('foo')
 * // => true
 * ```
 *
 * @method enabled
 * @param {String} `key`
 * @return {Boolean}
 * @api public
 */

Config.prototype.enabled = function enabled(key){
  return !!this.get(key);
};


/**
 * ## .disabled (key)
 *
 * Check if `key` is disabled.
 *
 * ```js
 * assemble.disabled('foo')
 * // => true
 *
 * assemble.enable('foo')
 * assemble.disabled('foo')
 * // => false
 * ```
 *
 * @method disabled
 * @param {String} `key`
 * @return {Boolean}
 * @api public
 */

Config.prototype.disabled = function disabled(key){
  return !this.get(key);
};


/**
 * ## .enable (key)
 *
 * Enable `key`.
 *
 * **Example**
 *
 * ```js
 * assemble.enable('foo');
 * ```
 *
 * @method enable
 * @param {String} `key`
 * @return {Config} for chaining
 * @api public
 */

Config.prototype.enable = function enable(key){
  return this.set(key, true);
};


/**
 * ## .disable (key)
 *
 * Disable `key`.
 *
 * **Example**
 *
 * ```js
 * assemble.disable('foo');
 * ```
 *
 * @method disable
 * @param {String} `key`
 * @return {Config} for chaining
 * @api public
 */

Config.prototype.disable = function disable(key){
  return this.set(key, false);
};


/*
 * ## .exists
 *
 * Return `true` if the element exists. Dot notation may be used for nested properties.
 *
 * **Example**
 *
 * ```js
 * assemble.exists('author.name');
 * //=> true
 * ```
 *
 * @method  `exists`
 * @param   {String}  `key`
 * @return  {Boolean}
 * @api public
 */

Config.prototype.exists = function exists(key) {
  return getobject.exists(this.cache, key);
};


/**
 * ## .extend
 *
 * Extend the config with the given object. This method is chainable.
 *
 * **Example**
 *
 * ```js
 * assemble
 *   .extend({foo: 'bar'}, {baz: 'quux'});
 *   .extend({fez: 'bang'});
 * ```
 *
 * @chainable
 * @method extend
 * @return {Config} for chaining
 * @api public
 */

Config.prototype.extend = function extend() {
  var args = [].slice.call(arguments);
  _.extend.apply(_, [this.orig].concat(args));
  this.emit('extend');
  return this;
};

Config.prototype.config = Config.prototype.extend;


/**
 * ## .merge
 *
 * Extend the config with the given object. This method is chainable.
 *
 * **Example**
 *
 * ```js
 * assemble
 *   .merge({foo: 'bar'}, {baz: 'quux'});
 *   .merge({fez: 'bang'});
 * ```
 *
 * @chainable
 * @method merge
 * @return {Config} for chaining
 * @api public
 */

Config.prototype.merge = function merge() {
  var args = [].slice.call(arguments);
  _.merge.apply(_, [this.orig].concat(args));
  this.emit('merge');
  return this;
};


/**
 * ## .process
 *
 * Use [expander] to recursively expand template strings into
 * their resolved values.
 *
 * **Example**
 *
 * ```js
 * assemble.process({a: '<%= b %>', b: 'c'});
 * //=> {a: 'c', b: 'c'}
 * ```
 *
 * @param {*} `lookup` Any value to process, usually strings with a
 *                     config template, like `<%= foo %>` or `${foo}`.
 * @param {Any} `opts` Options to pass to Lo-Dash `_.template`.
 * @api public
 */

Config.prototype.process = function process(lookup, context) {
  if (typeOf(lookup) === 'object') {
    this.extend(lookup);
  }

  context = context || this.cache;
  var imports = {};

  for (var key in context) {
    if (context.hasOwnProperty(key)) {
      if (typeof key === 'function') {
        imports[key] = context[key];
      }
    }
  }

  return this.extend(expand(context, lookup, {
    imports: imports
  }));
};


/**
 * ## Clearing the orig
 *
 * > Methods for clearing the orig, or for removing or reseting specific values on the orig.
 *
 */


/**
 * ## .omit
 *
 * Omit properties and their from the orig.
 *
 * **Example:**
 *
 * ```js
 * assemble
 *   .omit('foo');
 *   .omit('foo', 'bar');
 *   .omit(['foo']);
 *   .omit(['foo', 'bar']);
 * ```
 *
 * @chainable
 * @method `omit`
 * @return {Config} for chaining
 * @api public
 */

Config.prototype.omit = function omit() {
  var args = [].slice.call(arguments);
  this.orig = _.omit.apply(_, [this.orig].concat(args));
  this.emit('omit');
  return this;
};


/**
 * ## .clear
 *
 * Remove `key` from the orig, or if no value is specified the entire
 * orig is reset.
 *
 * **Example:**
 *
 * ```js
 * assemble.clear();
 * ```
 *
 * @chainable
 * @method `clear`
 * @return {Config} for chaining
 * @api public
 */

Config.prototype.clear = function clear(key) {
  if (!key) {
    this.orig = {};
    this.emit('clear');
  } else if (this.orig[key]) {
    delete this.orig[key];
    this.emit('clear', key);
  }
  return this;
};


module.exports = Config;