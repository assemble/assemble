...
module.exports = function (file, encoding, options) {
  ...
  try {
    ...
    file.src = _.extend(file.src, {
      assets: file.assets || opts.assets || null,
      base: opts.src.base || file.base,
      cwd: opts.src.cwd || file.cwd,
      path: file.path,
      ext: opts.src.ext || _.last(parsed.extSegments) || parsed.extname
    });

    // Dest
    file.dest = _.extend(file.dest, {
      assets: file.assets || opts.assets || null,
      root: opts.root || file.root || file.dirname,
      path: fn ? fn(file.path) : file.path,
      ext: opts.dest.ext || _.last(parsed.extSegments) || parsed.extname,
    });
  } catch(err) {
    throw new Error('Assemble#utils.parse-path', err);
  }
