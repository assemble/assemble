...
* Render templates using any template engine. You can even use multiple engines at once, Assemble will automatically detect the correct one to use on each template at render time.
...
* Assemble gives you more granular control over context and all of the stages in the [render cycle](#render-cycle) than **any** other site generator
* Assemble is built on [base][], so if you learn how to use assemble, you will know how to use other applications built on `base`, like [templates][], [verb][], [generate][] and [update][]
...
### What can I create with assemble?

You can use assemble for:
